  ...
  <ying-page-wapper>
    <ying-table :service="service" title="栏目列表" ref="table">
      ...
      <template slot="batch-action" slot-scope="{rows}">
        <a-button type="primary" @click="modal().form().then(()=>{this.$refs.table.refresh()})">
          <a-icon type="plus" />创建
        </a-button>
        <a-button type="danger" @click="modal().batchDelete({rows:rows}).then(()=>{this.$refs.table.refresh()})">
          <a-icon type="delete" />删除
        </a-button>
      </template>
      <template slot="action" slot-scope="{record}">
        <a-button type="link" @click="modal().form({model:record,create:false}).then(()=>{this.$refs.table.refresh()})">编辑</a-button>
        <a-button type="link" @click="modal().delete({model:record}).then(()=>{this.$refs.table.refresh()})">删除</a-button>
      </template>
    </ying-table>
  </ying-page-wapper>
</template>
